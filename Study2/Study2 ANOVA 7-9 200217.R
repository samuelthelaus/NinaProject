...
ANOVA_7_duet$log_ball <- log(ANOVA_7_duet$BallDist)

meansSubject7 <- aggregate(log_ball ~ Bin + Subject, ANOVA_7_duet, mean)

model_7_1 <- aov(log_ball ~ Bin + Error(Subject), data = meansSubject7)
summary(model_7_1)

sum(ANOVA_int_duet$Length[ANOVA_int_duet$Segment == 'Listen'] > 1000)/
  sum(ANOVA_int_duet$Segment == 'Listen')
...
ggsave('line_graph_ANOVA_7_duet.png', width = 6, height = 4.5)
...
ANOVA_8_duet$log_ball <- log(ANOVA_8_duet$BallDist)

meansSubject8 <- aggregate(log_ball ~ Bin + Subject, ANOVA_8_duet, mean)

model_8_1 <- aov(log_ball ~ Bin + Error(Subject), data = meansSubject8)
summary(model_8_1)

sum(ANOVA_int_duet$Length[ANOVA_int_duet$Segment == 'Prepare'] > 500)/
  sum(ANOVA_int_duet$Segment == 'Prepare')
...
ggsave('line_graph_ANOVA_8_duet.png', width = 6, height = 4.5)
...
ggsave('line_graph_ANOVA_79_duet.png', width = 6, height = 4.5)


#### Number 5 ####

ANOVA_9_duet$log_ball <- log(ANOVA_9_duet$BallDist)

meansSubject9 <- aggregate(log_ball ~ Bin + Subject, ANOVA_9_duet, mean)

model_9_1 <- aov(log_ball ~ Bin + Error(Subject), data = meansSubject9)
summary(model_9_1)

sum(ANOVA_int_duet$Length[ANOVA_int_duet$Segment == 'Listen'] > 1440)/
  sum(ANOVA_int_duet$Segment == 'Listen')
...
ggsave('line_graph_ANOVA_9_duet.png', width = 6, height = 4.5)
