...
ANOVA_4_duet$log_ball <- log(ANOVA_4_duet$BallDist)

meansSubject3 <- aggregate(log_ball ~ Segment + Familiarity + Subject, ANOVA_4_duet, mean)

model_4_1 <- aov(log_ball ~ Segment + Familiarity + Error(Subject), data = meansSubject3)
summary(model_4_1)

save_desc_3 <- data.frame()
for(i in levels(ANOVA_4_duet$Segment)) {
  mean_seg <- mean(ANOVA_4_duet$log_ball[ANOVA_4_duet$Segment == i])
  sd_seg <- sd(ANOVA_4_duet$log_ball[ANOVA_4_duet$Segment == i])
...
for(i in levels(ANOVA_4_duet$Segment)) {
  ...
  x <- cbind(i, as.data.frame(psych::describe(ANOVA_4_duet$log_ball[ANOVA_4_duet$Segment == i]))[, c(2:4, 8, 9, 11)])
...
write.csv(desc_means_3, 'C:/Users/samuel/Google Drive/Uni/Year 4/VRA Edinburgh/Study 2/Dump/desc_means_3.csv', row.names = F)
...
ggsave('line_graph_ANOVA_4_duet.png', width = 5, height = 4)

# Frequencies of Segments
summary(ANOVA_4_duet$Segment)


#### ANOVA 5 Script ####

ANOVA_5_duet$log_ball <- log(ANOVA_5_duet$BallDist)

meansSubject4 <- aggregate(log_ball ~ Segment + Familiarity + Subject, ANOVA_5_duet, mean)

model_5_1 <- aov(log_ball ~ Segment + Familiarity + Error(Subject), data = meansSubject4)
summary(model_5_1)

save_desc_4 <- data.frame()
for(i in levels(ANOVA_5_duet$Segment)) {
  mean_seg <- mean(ANOVA_5_duet$log_ball[ANOVA_5_duet$Segment == i])
  sd_seg <- sd(ANOVA_5_duet$log_ball[ANOVA_5_duet$Segment == i])
...
for(i in levels(ANOVA_5_duet$Segment)) {
  ...
  x <- cbind(i, as.data.frame(psych::describe(ANOVA_5_duet$log_ball[ANOVA_5_duet$Segment == i]))[, c(2:4, 8, 9, 11)])
...
write.csv(desc_means_4, 'C:/Users/samuel/Google Drive/Uni/Year 4/VRA Edinburgh/Study 2/Dump/desc_means_4_duets.csv', row.names = F)
...
ggsave('line_graph_ANOVA_5_duet.png', width = 5, height = 4)

# Frequencies of Segments
summary(ANOVA_5_duet$Segment)
...
ggsave('line_graph_ANOVA_6_duets.png', width = 5, height = 4)


ANOVA_6 <- rbind(ANOVA_4_duet, ANOVA_5_duet)
...
write.csv(desc_means_5, 'C:/Users/samuel/Google Drive/Uni/Year 4/VRA Edinburgh/Study 2/Dump/desc_means_5_duets.csv', row.names = F)
