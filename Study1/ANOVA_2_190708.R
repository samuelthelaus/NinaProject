...
ANOVA_2$log_ball <- log(ANOVA_2$BallDist)

meansSubject2 <- aggregate(log_ball ~ Segment + Subject, ANOVA_2, mean)

model_2_1 <- aov(log_ball ~ Segment + Error(Subject), data = meansSubject2)
summary(model_2_1)

save_desc_2 <- data.frame()
for(i in levels(ANOVA_2$Segment)) {
  mean_seg <- mean(ANOVA_2$log_ball[ANOVA_2$Segment == i])
  sd_seg <- sd(ANOVA_2$log_ball[ANOVA_2$Segment == i])
...
for(i in levels(ANOVA_2$Segment)) {
  ...
  x <- cbind(i, as.data.frame(psych::describe(ANOVA_2$log_ball[ANOVA_2$Segment == i]))[, c(2:4, 8, 9, 11)])
...
SumANOVA_seg <- data.frame(Segment = factor(c(1, 2, 3, 4), levels = c(1, 2, 3, 4),
                           ...
                           log_ball = c(mean(ANOVA_2$log_ball[ANOVA_2$Segment == 'Play']),
                                        mean(ANOVA_2$log_ball[ANOVA_2$Segment == 'Wait']),
                                        mean(ANOVA_2$log_ball[ANOVA_2$Segment == 'Listen']),
                                        mean(ANOVA_2$log_ball[ANOVA_2$Segment == 'Prepare'])),
                           se = c(se(ANOVA_2$log_ball[ANOVA_2$Segment == 'Play']),
                                  se(ANOVA_2$log_ball[ANOVA_2$Segment == 'Wait']),
                                  se(ANOVA_2$log_ball[ANOVA_2$Segment == 'Listen']),
                                  se(ANOVA_2$log_ball[ANOVA_2$Segment == 'Prepare'])))
